...
class RecordedAdmin(admin.ModelAdmin):
    list_display = ('program', 'file', 'started_at',
                    'last_updated_at', 'ended_at', 'is_recording')
    search_fields = ('program__title', 'file')
    list_filter = ('is_recording', 'started_at',
                   'ended_at', 'program__service_id')
    readonly_fields = ('program', 'file', 'started_at',
                       'last_updated_at', 'ended_at', 'is_recording')
    fieldsets = (
        (None, {
            'fields': ('program', 'file', 'started_at', 'last_updated_at', 'ended_at', 'is_recording')
        }),
    )
